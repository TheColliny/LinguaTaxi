...
Handles audio file loading with resampling, silence-based segmentation,
batch transcription/translation pipelines, and real-time file playback
into the audio source queue.
"""
...
AUDIO_EXTS: set[str] = {".wav", ".mp3", ".flac", ".m4a", ".ogg", ".webm"}
TEXT_EXTS: set[str] = {".txt", ".srt", ".vtt", ".md"}
MAX_TEXT_FILE_SIZE: int = 10 * 1024 * 1024  # 10MB
...
def _sanitize_text_line(line: str) -> str:
    ...
    Args:
        line: Raw text line to sanitize.

    Returns:
        Sanitized line, truncated to :data:`MAX_LINE_LENGTH`.
    """
...
def load_audio(file_path: str | Path) -> tuple[np.ndarray, float]:
    ...
    Returns:
        Tuple of ``(samples, duration_sec)``.

    Raises:
        ValueError: For unsupported formats or empty files.
    """
    p = Path(file_path)
    ext = p.suffix.lower()
...
def segment_audio(
    ...
    Uses the same algorithm as the server's ``_buffer_audio_loop``.

    Args:
        samples: Audio samples (16kHz mono float32).
        silence_threshold: RMS threshold below which audio is silence.
        silence_duration: Duration of silence required to split (seconds).
        max_segment_duration: Maximum segment length before forced split (seconds).
...
def get_progress() -> dict[str, Any]:
    ...
    Returns:
        Dict with ``status``, ``pct``, ``message``, ``current_file``,
...
def _set_progress(
    ...
    """Update batch processing progress state.

    Args:
        status: Status string (``"processing"``, ``"done"``, ``"error"``, etc.).
        ...
        files_total: Total number of files in the batch.
    """
    with _progress_lock:
...
def batch_translate_text(
    ...
    _emit_done: bool = True,
) -> dict[str, Any] | None:
    """Translate a text file to multiple languages.

    Args:
        file_path: Path to the source text file.
        translate_fn: Translation function ``(text, target_lang, source_lang, mode) -> str``.
        translations: List of translation slot dicts ``{"lang": "ES", "mode": "deepl"}``.
        output_dir: Directory for translated output files.
        source_lang: Source language code.
        progress_callback: Optional ``(pct, message)`` callback.
        _emit_done: Whether to emit a ``"done"`` progress state on completion.

    Returns:
        Dict with ``lines``, ``languages``, ``output_dir``, or ``None`` on error.
    """
    p = Path(file_path)
...
def _transcribe_segment_vosk(segment: np.ndarray, vosk_model: Any) -> str:
    """Transcribe a single segment using a temporary Vosk KaldiRecognizer.

    Args:
        segment: Audio segment (16kHz mono float32).
        vosk_model: A loaded Vosk model instance.
...
def batch_transcribe(
    ...
    Args:
        file_path: Path to the audio file.
        stt_backend: Active SpeechBackend instance (WhisperBackend or VoskBackend).
        translate_fn: Translation function.
        translations: List of translation slot dicts.
    ...
    Returns:
        Dict with ``lines``, ``duration_sec``, ``output_dir``, ``languages``,
        or ``None`` on error.
    """
    ...
    languages: list[str] = [source_lang]
...
def batch_folder(
    ...
    Args:
        ...
        stt_backend: Active SpeechBackend instance.
        translate_fn: Translation function.
        translations: List of translation slot dicts.
    ...
    Returns:
        Dict with ``files_processed``, ``files_skipped``, ``total_lines``,
        ``languages``, ``output_dir``, or ``None`` on error.
    """
    ...
    for idx, fp in enumerate(files):
        ...
        # Reset progress to folder-level
        _set_progress("processing", file_pct,
                      f"Processing {fp.name} ({idx + 1}/{total})",
                      current_file=fp.name, files_done=idx, files_total=total)
...
def start_live_playback(
    ...
    Returns:
        A :class:`threading.Event` that can be set to cancel playback.

    Raises:
        RuntimeError: If a file transcription is already in progress.
    """
    ...
    chunk_size = int(SAMPLE_RATE * 0.5)  # 8000 samples = 0.5s
