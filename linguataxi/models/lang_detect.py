...
log: logging.Logger = logging.getLogger("lang_detect")

# Default model storage alongside other models
_MODELS_DIR: Path = Path(__file__).parent.parent.parent / "models"
_MODEL_SUBDIR: str = "silero-lang-detect"

# The actual filename is lang_classifier_95.onnx (not lang_detector_95).
_MODEL_FILENAME: str = "lang_classifier_95.onnx"
_DICT_FILENAME: str = "lang_dict_95.json"

_MODEL_URL: str = "https://huggingface.co/deepghs/silero-lang95-onnx/resolve/main/lang_classifier_95.onnx"
_LANG_DICT_URL: str = "https://huggingface.co/deepghs/silero-lang95-onnx/resolve/main/lang_dict_95.json"

_session: Any = None  # ONNX InferenceSession (lazy loaded)
_lang_dict: dict[int, str] | None = None  # {index: lang_code} mapping
_load_lock: threading.Lock = threading.Lock()
_load_failed: bool = False  # Set True after download/load failure to prevent retry spam
...
def _model_dir() -> Path:
    """Return the path to the language detection model subdirectory.
...
def download_model(models_dir: str | Path | None = None) -> tuple[Path, Path]:
...
def _load() -> None:
    ...
    Raises:
        RuntimeError: If the model is unavailable due to a previous load failure.
    """
