#!/usr/bin/env python3
"""
LinguaTaxi — Model Download Manager

Orchestrates model downloads with unified progress reporting.
Used by the Inno Setup installer (polls the JSON status file) and
importable by the runtime for model update checks.
...
def _run_download_script(python: str, script_args: list[str], label: str,
                         task_id: str):
...
def run_plan(plan: dict, progress_path: str | None = None):
...
def _load_remote_manifest(timeout=10):
    try:
...
def _write_installed_version(model_dir: Path, version: str):
    vf = model_dir / "_version.json"
    try:
...
def check_updates(models_dir: str, progress_path: str | None = None) -> list[dict]:
    """Compare installed models against the remote manifest.
    Returns a list of {model_id, installed_version, available_version, size_mb}."""
    global _progress_path
    _progress_path = progress_path
...
def stamp_installed_models(models_dir: str):
